...
int* IndexSort(Student* p, const int N)
{
    ...
    for (i = 1; i < N; i++)
    {
        ...
        for (j = i - 1;
            (j >= 0 && ((p[I[j]].physics + p[I[j]].math + p[I[j]].IT) > (p[value].physics + p[value].math + p[value].IT)
                ||
...
int BinSearch(Student* p, const int N, const string prizv, const unsigned int kurs, const double avgGrade)
{
    ...
    do {
        ...
        if (((p[m].physics + p[m].math + p[m].IT) / 3.0) < avgGrade
            ||
        (((p[m].physics + p[m].math + p[m].IT) / 3.0) == avgGrade && p[m].kurs < kurs)
            ||
            (p[m].prizv == prizv && p[m].kurs == kurs && p[m].prizv < prizv))
        {
            L = m + 1;
        }
        else
